...
use crate::app::balloon_control::BalloonControl;
use crate::app::polyp::{PolypRemoval, PolypTelemetry};
use crate::app::tunnel::{advance_centerline, tunnel_centerline, tunnel_tangent_rotation};
...
pub fn peristaltic_drive(
    ...
    *body_rb = if balloon.tail_inflated {
    ...
    if let Some(anchor_pos) = head_anchor_pos {
        let max_z = crate::app::tunnel::TUNNEL_START_Z + crate::app::tunnel::TUNNEL_LENGTH;
        let mut low = (anchor_pos.z - length - 5.0).clamp(crate::app::tunnel::TUNNEL_START_Z, max_z);
        let mut high = (anchor_pos.z + 5.0).clamp(crate::app::tunnel::TUNNEL_START_Z, max_z);
    ...
    if let Ok(mut cam_tf) = transforms.p4().single_mut() {
        let max_arc = (crate::app::tunnel::TUNNEL_START_Z + crate::app::tunnel::TUNNEL_LENGTH
            - params.tail_z)
            .max(length);
        let look_ahead = 0.5;
