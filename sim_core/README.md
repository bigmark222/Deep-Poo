...
- Plugins: SimPlugin (mode sets), SimRuntimePlugin (default runtime resources), SimSystemsPlugin (wires existing sim/autopilot/recorder systems).
- Modules: camera/controls, autopilot_types/recorder_types, probe_types.

How to use
1) Build the base app via `sim_core::build_app(SimConfig { .. })`.
2) Add plugins: `SimPlugin`, `SimRuntimePlugin`, `SimSystemsPlugin`, plus any app-specific bootstrap (e.g., environment).
...
Use `SimRuntimePlugin` to keep registration in one place; avoid detector wiring here to keep the core crate lean.
